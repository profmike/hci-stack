...
```bash
_V=$(cat ~/.claude/skills/hci-office-hours-with-mike/VERSION 2>/dev/null || echo "unknown")
...
```bash
_LOCAL=$(cat ~/.claude/skills/hci-office-hours-with-mike/VERSION 2>/dev/null || echo "unknown")
...
```bash
if [ -d ~/.claude/skills/hci-office-hours-with-mike/.git ]; then
  cd ~/.claude/skills/hci-office-hours-with-mike && git pull origin main
else
  curl -sf "https://raw.githubusercontent.com/profmike/hci-stack/main/hci-office-hours-with-mike/SKILL.md" \
    -o ~/.claude/skills/hci-office-hours-with-mike/SKILL.md
  curl -sf "https://raw.githubusercontent.com/profmike/hci-stack/main/VERSION" \
    -o ~/.claude/skills/hci-office-hours-with-mike/VERSION
fi
```
